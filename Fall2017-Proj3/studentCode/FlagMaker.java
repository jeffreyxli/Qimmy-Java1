...
public class FlagMaker {
	...
	public static void drawFlag(Grid_3x5 grid, int flagCode) {
		...
		else if(flagCode==5 && grid.getWd()%2==1) {
			drawSanta(grid);
		}
	...
	private static void drawPrague(Grid_3x5 grid)
	{
	...
	{
		helperField(grid, Color.RED);
		for(int row=grid.getHt()/3; row<grid.getHt()*2/3; row++)
		{
			for(int col=0; col < grid.getWd(); col++)
			{
	...
	private static void drawNorfolk(Grid_3x5 grid)
	{
		...
		for(int row=0; row<grid.getHt(); row++)
		{
			for(int col=grid.getWd()*3/10; col<grid.getWd()*7/10; col++)
			{
				grid.setColor(row, col, Color.WHITE);
			}
		}
	...
	private static void drawVincent(Grid_3x5 grid)
	{
		...
		for(int row=0; row<grid.getHt(); row++)
		{
			for(int col=0; col<grid.getWd()*3/10; col++)
			{
	...
	private static void drawHour(Grid_3x5 grid)
	{
		helperField(grid, Color.GREEN);
		int counter = 0;
		for(int row = 0; row<=grid.getHt()/2; row++)
		{
			for(int col=0; col<grid.getWd(); col++)
			{
				if(col<=counter && counter < grid.getWd()/3)
				{
		...
		int counterTwo = (grid.getHt()/2);
		for(int row = grid.getHt()/2; row<grid.getHt(); row++)
		{
			for(int col=0; col<grid.getWd(); col++)
			{
				if(col<=counterTwo && counterTwo >= 0)
				{
					grid.setColor(row, col, Color.BLACK);
				}
			}
		...
		int counterThree = grid.getWd()-((grid.getHt()/2)+1);
		for(int row = grid.getHt()/2; row>=0; row--)
		{
			for(int col=grid.getWd()-1; col>=0; col--)
			{
				if(col>=counterThree && counterThree < grid.getWd())
				{
					grid.setColor(row, col, Color.BLACK);
				}
			}
		...
		int counterFour = grid.getWd()-((grid.getHt()/2)+1); 
		for(int row = grid.getHt()/2; row<grid.getHt(); row++)
		{
			for(int col=grid.getWd()-1; col>=0; col--)
			{
				if(col>=counterFour && counterFour < grid.getWd())
				{
					grid.setColor(row, col, Color.BLACK);
				}
			}
