...
public class Novelization implements Critiqueable {
	...
	public int getStarPower() {
		return stars + (title.charAt(0)%10);
	}
	...
	public boolean inform(Universe.Outcomes outcome) {
		...
		if (outcome.equals(Universe.Outcomes.FRESH)) {
			winsSinceStarBump++;
		}
		else if(outcome.equals(Universe.Outcomes.ROTTEN))
		{
			numberOfFans = 0;
		}
	...
	public Critiqueable returnClone() {
		Novelization copy = new Novelization(title, numberOfFans);
		return copy;
	}
